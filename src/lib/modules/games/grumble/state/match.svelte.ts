/**
 * Single source of truth for a Grumble match. Lives in a .svelte.ts module so
 * the runes work outside a component — import `match` anywhere and read its getters.
 *
 ...
 */
import { finalScore, isGameOver, leader, tallyHands } from '../scoring/score.js';
import { DEFAULT_RULES, PRESETS, type PresetId, type Rules } from '../scoring/rules.js';
import type { GameRecord, Hand, PlayerId } from '../scoring/types.js';
import { loadMatch, saveMatch } from './persistence.js';
...
function createMatch() {
	...
	function persist() {
		saveMatch({ names, hands, games, rules, presetId });
	}
